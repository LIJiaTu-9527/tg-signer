...
def load_llm_config(workdir: Optional[Path | str] = None) -> Optional[LLMConfigEntry]:
    manager = OpenAIConfigManager(get_workdir(workdir))
    ...
    return LLMConfigEntry(
        ...
        from_env=manager.has_env_config(),
    )
...
def save_llm_config(
    ...
    manager = OpenAIConfigManager(get_workdir(workdir))
    manager.save_config(api_key, base_url=base_url, model=model or DEFAULT_MODEL)
    return manager.get_config_file()


def list_session_accounts(session_dir: Optional[Path | str] = None) -> List[str]:
    base = Path(session_dir or ".").expanduser()
    if not base.is_dir():
        return []
    accounts = {path.stem for path in base.glob("*.session") if path.is_file()}
    accounts.update(
        path.name[: -len(".session_string")]
        for path in base.glob("*.session_string")
        if path.is_file()
    )
    return sorted(account for account in accounts if account)


def _keepalive_config_path(workdir: Optional[Path | str] = None) -> Path:
    return get_workdir(workdir) / "webui_keepalive.json"


def load_keepalive_config(
    workdir: Optional[Path | str] = None,
) -> Optional[KeepaliveRunConfig]:
    path = _keepalive_config_path(workdir)
    if not path.is_file():
        return None
    with open(path, "r", encoding="utf-8") as fp:
        data = json.load(fp)
    return KeepaliveRunConfig(
        account=data.get("account") or "my_account",
        signer_tasks=list(data.get("signer_tasks") or []),
        monitor_tasks=list(data.get("monitor_tasks") or []),
    )
...
def save_keepalive_config(
    ...
    path = _keepalive_config_path(workdir)
    payload = {
        "account": account.strip() or "my_account",
        "signer_tasks": [task.strip() for task in signer_tasks if task.strip()],
        "monitor_tasks": [task.strip() for task in monitor_tasks if task.strip()],
    }
