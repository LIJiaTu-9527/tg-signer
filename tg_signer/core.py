...
class BaseUserWorker(Generic[ConfigT]):
    ...
    async def _load_latest_chats(self, num_of_dialogs=20, print_chat=True):
        latest_chats = []
        async for dialog in self.app.get_dialogs(limit=num_of_dialogs):
            chat = dialog.chat
            latest_chats.append(
                {
                    "id": chat.id,
    ...
    async def sync_login_state(self, num_of_dialogs=20, print_chat=True):
        me = await self._call_telegram_api("users.GetFullUser", self.app.get_me)
        latest_chats = await self._call_telegram_api(
            "messages.GetDialogs",
            lambda: self._load_latest_chats(
    ...
    async def login(self, num_of_dialogs=20, print_chat=True):
        ...
        async with lock:
            ...
            if me is None:
                async with app:
                    me = await self.sync_login_state(
                        num_of_dialogs=num_of_dialogs,
                        print_chat=print_chat,
                    )
            else:
...
class UserSigner(BaseUserWorker[SignConfigV3]):
    ...
    async def normal_run(
        ...
        message_handler = MessageHandler(self.on_message, filters.chat(chat_ids))
        edited_handler = EditedMessageHandler(
        ...
        self.app.add_handler(edited_handler)
        try:
            while True:
                try:
                    async with self.app:
            ...
            self.app.remove_handler(message_handler)
            self.app.remove_handler(edited_handler)
    ...
    async def get_schedule_messages_data(self, chat_id):
        if self.user is None:
            await self.login(print_chat=False)
        async with self.app:
            messages = await self._call_telegram_api(
...
class UserMonitor(BaseUserWorker[MonitorConfig]):
    ...
    async def run(self, num_of_dialogs=20):
        ...
        message_handler = MessageHandler(
            self.on_message, filters.text & filters.chat(cfg.chat_ids)
        )
        self.app.add_handler(message_handler)
        try:
            async with self.app:
